...
// 实时进度条：扫描包按阶段(已知/自动发现/通用识别)回传 done/total，这里换算为整体百分比
...
function renderList() {
  ...
  entries.forEach((e, i) => {
    ...
    row.innerHTML =
      ...
      `<span class="badge ${riskClass(e.risk)}">${e.risk}</span>` +
      `<span class="cat">${e.category}</span>` +
      `<span class="path" title="${e.path}">${e.shortPath}</span>`;
...
async function refreshDirs() {
  ...
  $('#custom-list').innerHTML = (custom && custom.length)
    ? custom.map((p) => `<li>${p} <button class="x" data-type="custom" data-p="${p}">✕</button></li>`).join('')
    : '<li class="empty">（空）</li>';
  $('#exclude-list').innerHTML = (exclude && exclude.length)
    ? exclude.map((p) => `<li>${p} <button class="x" data-type="exclude" data-p="${p}">✕</button></li>`).join('')
    : '<li class="empty">（空）</li>';
}
...
async function refreshHistory() {
  ...
  $('#history-body').innerHTML = (h && h.length)
    ? h.map((r) => `<tr><td>${r.time}</td><td>${fmtSize(r.size)}</td><td title="${r.path}">${r.path}</td></tr>`).join('')
